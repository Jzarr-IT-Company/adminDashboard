...
function FaqsLogin() {
  const { setFaqsToken } = useGlobalState()
  ...
  const handleLogin = async () => {
    try {
      const response = await axios.post('http://localhost:4040/faqsLoginPasswordCheck', { password, email });

      if (response.data.status === 200) {
        alert("SUUCESSFULLY")
        setFaqsToken(response.data.token)
        Cookies.set('physId', response.data.id);
        Cookies.set('physToken', response.data.token);
        navigate('/faqs');
      }
    } catch (error) {
